...
%------------------------------------------------------------
\documentclass[draft,ebook]{longnotes}
\usepackage{tmath} 
\usepackage{cussymb} 
\usepackage{silence}
...
по методичке Лодкина\quest

\begin{defn}\label{defn:dg::vecfione::basic}
  Пусть $f$~--- скалярное поле, $F = (P,Q,R)$~--- векторное. Тогда
  \begin{align*}
    \grad f &= \left( \pder f x, \pder f y , \pder f z \right) \\
    \rot F  &= \left( \pder R y - \pder Q z, \pder P z - \pder R x , \pder Q x - \pder P y \right) \\
    \div F  &= \pder P x + \pder Q y + \pder R z
  \end{align*}
\end{defn}
...
\begin{tproof}
  ...
  по вертикальным краям интеграл обратится в ноль.

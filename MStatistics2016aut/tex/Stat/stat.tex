...
\begin{defn}\label{defn:stat::fishlem::chi}
  ...
  Тогда $\sum_i X_i^2$ имеет распределение $\chi^2$ c $n$ степенями свободы. 
\end{defn}
...
\begin{prop}\label{prop:stat::fishlem::studens}
  Плотность распределения $\chi^2_n$ ищется по формуле 
  $\displaystyle s_n = \dfrac{\Gamma(\frac{n+1}{2})}{\Gamma(\frac{n}{2}) \sqrt{\pi n}} 
...
\begin{lem}[Фишера]\label{lem:stat::fishlem::fishlem}
  ...
  \begin{enumerate}
    ...
    \item $\displaystyle \sqrt{n-1}\, \frac{\overline X - \theta }{S}  \sim S_{n-1}$
  \end{enumerate}
\end{lem}
\begin{ittproof}
  \begin{enumerate}
    ...
    \item Пусть
      ...
      набрать ортогональный базис. Тогда $C$ будет ортогональным. Пусть $Y = CX$. Из горы
